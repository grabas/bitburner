...
import { UniquePathsOnGrid2 } from "./solvers/unique-paths-on-grind-2";
import { MinimumPathSumInTriangle } from "./solvers/minimum-path-sum-in-triangle";
import { UniquePathsOnGrid } from "./solvers/unique-paths-on-grind";
...
const solvers = [
    ...
    new VigenereCipher(),
    new SquareRoot(),
    new UniquePathsOnGrid(),
    new UniquePathsOnGrid2(),
...
const solverMap = new Map<keyof CodingContractSignatures, ISolver<keyof CodingContractSignatures>>();

solvers.forEach(solver => solverMap.set(solver.getType(), solver as ISolver<keyof CodingContractSignatures>));

export function getSolver<T extends keyof CodingContractSignatures>(
    solverType: T
): ISolver<T> | undefined {
    return solverMap.get(solverType) as ISolver<T> | undefined;
}
...
import {NS} from "@ns";
import {DivisionConfig, StrategyConfig} from "/lib/component/corporation/strategy/round-2/round-2.config";
import {StrategyBase} from "/lib/component/corporation/strategy/strategy.base";
import {RoundFinalStrategy} from "/lib/component/corporation/strategy/round-final/round-final.strategy";

export class Round2Strategy extends StrategyBase implements IStrategy {
    constructor(ns: NS) {
       super(ns);
    }
    ...
    public getNextStrategy(): IStrategy | null {
        return new RoundFinalStrategy(this.ns);
    }

    public async finalStep(): Promise<void> {
        if (this.corporation.isPublic()) return

        this.corporationApi.goPublic(1);
        this.corporationApi.issueDividends(0.1);
    }
}